  ...
  textureLoader.load(
    ...
    (loadedTexture) => {
      try {
        ...
        // -------------------------------------------------------------
        // 2. Tapa lateral (Tapa: x=0.420, y=0.308, width=0.158, height=0.082)
        // -------------------------------------------------------------
        if (targetCapGroup && capMesh) {
          const sx = img.width * 0.420;
          const sy = img.height * 0.308;
          const sWidth = img.width * 0.158;
          const sHeight = img.height * 0.082;
        ...
        // -------------------------------------------------------------
        // 3. Cuello lateral (Cuello: x=0.433, y=0.387, width=0.102, height=0.076)
        // -------------------------------------------------------------
        if (cuelloMesh) {
          const sx = img.width * 0.433;
          const sy = img.height * 0.387;
          const sWidth = img.width * 0.102;
          const sHeight = img.height * 0.076;

          const canvasW = 256;
          const canvasH = Math.round(canvasW * (sHeight / sWidth));

          // Canvas original (derecho)
          const neckRightCanvas = document.createElement('canvas');
          neckRightCanvas.width = canvasW;
          neckRightCanvas.height = canvasH;
          const ctxR = neckRightCanvas.getContext('2d');
          ctxR.drawImage(img, sx, sy, sWidth, sHeight, 0, 0, canvasW, canvasH);

          // Canvas reflejado horizontalmente (izquierdo)
          const neckLeftCanvas = document.createElement('canvas');
          neckLeftCanvas.width = canvasW;
          neckLeftCanvas.height = canvasH;
          const ctxL = neckLeftCanvas.getContext('2d');
          ctxL.save();
          ctxL.scale(-1, 1);
          ctxL.drawImage(img, sx, sy, sWidth, sHeight, -canvasW, 0, canvasW, canvasH);
          ctxL.restore();

          neckRightCanvasTexture = createSideTexture(neckRightCanvas);
          neckLeftCanvasTexture = createSideTexture(neckLeftCanvas);

          neckRightPhotoMaterial = createSideMaterial(neckRightCanvasTexture);
          neckLeftPhotoMaterial = createSideMaterial(neckLeftCanvasTexture);

          const cuelloBox = new THREE.Box3().setFromObject(cuelloMesh);
          const cuelloSize = cuelloBox.getSize(new THREE.Vector3());
          const cuelloCenter = cuelloBox.getCenter(new THREE.Vector3());

          const sideWidth = cuelloSize.z * 0.98;
          const sideHeight = cuelloSize.y * 0.98;
          const neckSideGeo = new THREE.PlaneGeometry(sideWidth, sideHeight);

          // Lateral derecho (+X)
          neckRightPhotoProjection = new THREE.Mesh(neckSideGeo, neckRightPhotoMaterial);
          neckRightPhotoProjection.name = 'neckRightPhotoProjection';
          neckRightPhotoProjection.renderOrder = 4;
          neckRightPhotoProjection.position.set(cuelloBox.max.x + 0.00012, cuelloCenter.y, 0);
          neckRightPhotoProjection.rotation.y = Math.PI / 2;
          bottleGroup.add(neckRightPhotoProjection);

          // Lateral izquierdo (-X)
          neckLeftPhotoProjection = new THREE.Mesh(neckSideGeo, neckLeftPhotoMaterial);
          neckLeftPhotoProjection.name = 'neckLeftPhotoProjection';
          neckLeftPhotoProjection.renderOrder = 4;
          neckLeftPhotoProjection.position.set(cuelloBox.min.x - 0.00012, cuelloCenter.y, 0);
          neckLeftPhotoProjection.rotation.y = -Math.PI / 2;
          bottleGroup.add(neckLeftPhotoProjection);
        }

        // Intentar inicializar shader multivista cuando las texturas laterales estén listas
        tryInitBodyMultiviewShader();
      } catch (err) {
...
// 1. Proyección frontal (+Z)
vec2 uvFront = (normPos.xy - vec2(0.5)) / 0.985 + vec2(0.5);
...
vec2 uvRight = vec2(1.0 - ((normPos.z - 0.5) / 0.98 + 0.5), (normPos.y - 0.5) / 0.98 + 0.5);
vec4 colRight = sampleMultiviewPhoto(uTexRight, uvRight);

// 4. Proyección lateral izquierda (-X, reflejada horizontalmente)
vec2 uvLeft = vec2(((normPos.z - 0.5) / 0.98 + 0.5), (normPos.y - 0.5) / 0.98 + 0.5);
...
// Suavizado en cuello/hombro superior y base inferior
float horizFactor = smoothstep(0.96, 0.72, abs(n.y));
...
loader.load(
  ...
  (gltf) => {
    ...
    // Inicializar shader multivista si las texturas ya están preparadas
...
function animate() {
  ...
  if (bottleGroup) {
    ...
    // Proyecciones de tapa y cuello (conservadas y sincronizadas con la cámara)
    ...
    if (neckRightPhotoMaterial) neckRightPhotoMaterial.opacity = rightOpacity;
    if (capRightPhotoProjection) capRightPhotoProjection.visible = rightOpacity > 0.01;
    if (neckRightPhotoProjection) neckRightPhotoProjection.visible = rightOpacity > 0.01;

    // Lateral izquierdo (-X)
    if (capLeftPhotoMaterial) capLeftPhotoMaterial.opacity = leftOpacity;
    if (neckLeftPhotoMaterial) neckLeftPhotoMaterial.opacity = leftOpacity;
    if (capLeftPhotoProjection) capLeftPhotoProjection.visible = leftOpacity > 0.01;
    if (neckLeftPhotoProjection) neckLeftPhotoProjection.visible = leftOpacity > 0.01;
  }
